...
Nas arquiteturas IP, a comunicação é orientada ao nó de destino, ou seja, os pacotes de dados precisam de saber para onde ir, e não necessariamente o que transportam. Em ambientes veiculares, essa dependência de endereços fixos e de rotas estabelecidas resulta em latências elevadas, falhas frequentes de encaminhamento e baixa eficiência na disseminação de dados, especialmente em cenários de alta mobilidade ou desconexões momentâneas. Além disso, a necessidade constante de manutenção de tabelas de encaminhamento e reconfiguração de caminhos compromete a escalabilidade da rede.  

Neste contexto, o paradigma \gls{NDN} surge como uma alternativa disruptiva à arquitetura IP. O \gls{NDN} inverte o modelo clássico: em vez de focar no “onde” estão os dados, concentra-se naquilo que se pretende obter. Assim, a comunicação é centrada nos conteúdos. Por exemplo um veículo pode solicitar informações por nome (por exemplo, “/cidade/avenida/prioridade/alerta”) sem conhecer o endereço IP do nó que contem informações. O resultado é uma rede mais flexível, segura e eficiente, que aproveita o \textit{in-network caching}, a redundância natural dos dados e o encaminhamento por nome para melhorar a entrega da informação em ambientes dinâmicos.  
...
\begin{itemize}
    \item Analisar os principais mecanismos de sincronização do \gls{NDN}, com foco no \gls{SVSync}, identificando as suas limitações quando aplicados a ambientes de elevada veiculares.
    \item Propor uma versão adaptada do \gls{SVSync}, concebida para \gls{VANETs}, integrando fatores como mobilidade, restrições geográficas e temporais.
