...
\begin{figure}[H]
    ...
    \caption{Fluxograma das fases do processo de sincronização hierárquica no SVSync-H.}
    \label{fig:arq}
\end{figure}
...
\begin{algorithm}[H]
\caption{Processo de Sincronização Hierárquica SVSync-H}
\begin{algorithmic}[1]
...
As métricas a serem avaliadas no Capítulo~\ref{cha:resultados} permitem quantificar o desempenho do SVSync-H em comparação com o SVSync tradicional.  
...
A Figura \ref{fig:lnh} mostra a evolução da latência média em função do número de nós.  
...
\caption{Comparação da latência média entre as soluções sem hierarquia e hierárquica.}
\label{fig:lnh}
\end{figure}
A Tabela~\ref{tab:latencia_pivo} apresenta a comparação entre os valores de latência obtidos com e sem a utilização do pivô, para diferentes números de carros. Observa-se que, em todos os casos, a introdução do pivô resultou numa redução da latência média. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Avaliação do Consumo de Recursos}

Embora o foco principal seja o desempenho, existe também uma redução significativa no consumo de memória e processamento.  
O modelo hierárquico requer menos entradas na \gls{PIT} e reduz o uso da \gls{CS}, o que se traduz numa menor carga computacional por nó.  
Esta característica é essencial em redes heterogéneas, onde coexistem veículos com diferentes capacidades de hardware.
