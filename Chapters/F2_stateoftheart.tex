...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
A escolha de uma arquitetura baseada em pivôs para a sincronização de dados em redes veiculares NDN justifica-se pela necessidade de reduzir o overhead de mensagens e melhorar a escalabilidade em cenários urbanos densos. Os mecanismos tradicionais de sincronização, como o SVSync, embora eficazes em redes ad hoc com número moderado de participantes, enfrentam dificuldades quando aplicados a situações em que dezenas de veículos necessitam de partilhar estados quase em simultâneo. Nestes casos, a disseminação em malha completa provoca um crescimento quase quadrático do número de interesses e estados na Pending Interest Table, levando a colisões, perdas e sobrecarga de processamento nos nós móveis. A introdução de pivôs permite alterar este padrão de comunicação, uma vez que os veículos sincronizam apenas com um representante da sua faixa e esse representante, por sua vez, troca vetores de estado apenas com outros pivôs. Desta forma, a complexidade do processo reduz-se significativamente, aproximando-se de uma relação linear com o número de nós e limitada por um número pequeno de trocas entre pivôs. Esta abordagem não apenas minimiza o tráfego mas também aproveita as características geográficas do cenário, pois em cruzamentos urbanos existe uma segmentação natural por faixas, o que facilita a eleição de nós agregadores.

A integração com o SVSync reforça a adequação da solução, dado que este protocolo já foi desenhado para suportar sincronização assíncrona eficiente com codificação explícita de vetores de estado em cada interesse. Ao delegar em pivôs a emissão dos anúncios de estado para fora da faixa, evita-se que múltiplos produtores gerem respostas concorrentes, problema reconhecido nos trabalhos de análise sobre protocolos de sincronização distribuída em NDN. Além disso, a arquitetura mantém compatibilidade com os princípios de funcionamento do NDN, explorando a multicast por nome e a cache de conteúdos na rede, enquanto reduz a latência na convergência de estados locais. Esta escolha também equilibra robustez e simplicidade: embora os pivôs se tornem potenciais pontos de falha, a utilização de mecanismos de eleição rápida e anúncios periódicos mitiga este risco, assegurando continuidade na disseminação de dados mesmo perante mobilidade elevada.

Em síntese, a adoção de pivôs como agregadores de sincronização em conjunto com o SVSync permite conciliar a necessidade de escalabilidade com a manutenção de baixa latência e fiabilidade na comunicação, explorando de forma eficiente as propriedades específicas das redes veiculares. Esta estratégia surge alinhada com as limitações identificadas na literatura recente sobre sincronização em NDN e VANETs, sendo assim uma abordagem promissora para cenários urbanos de elevada densidade veicular
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Implementação}
\label{cha:implementação}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Inicialmente, são definidos os parâmetros de entrada que configuram intervalos de publicação rápidos e lentos, a quantidade de publicações recentes e aleatórias que devem ser consideradas pelo mecanismo de sincronização SVS, a taxa de perda de pacotes no canal e a possibilidade de fragmentação de pacotes. Em seguida, são configurados os dispositivos de rede e canais com um modelo de erro de taxa, largura de banda de 50 Mbps e atraso fixo de 5 ms. A topologia adotada é um grid de 5x5 nós interconectados por enlaces ponto a ponto. Sobre essa topologia, é instalada a pilha NDN em todos os nós, bem como tracers que permitem registar estatísticas de tráfego, atraso de aplicações e estado do Content Store. Cada nó da grelha, exceto o nó central (2,2), recebe uma aplicação de chat baseada em SVS. Cada aplicação tem um prefixo próprio e uma cadência de publicação que pode ser rápida ou lenta dependendo da paridade da posição do nó. Esses nós são também registados como origens de conteúdo para que possam ser alcançados através das rotas calculadas globalmente. Paralelamente, a classe HierarchicalSyncMobilityManager é responsável por orquestrar a mobilidade lógica dos nós, que consiste em movê-los ao ponto central. Cada nó possui metadados que incluem sua posição, um identificador lógico, a versão inicial dos dados e se ele atua como pivô ou provedor de dados. Os pivôs são definidos como os nós localizados na última coluna, e o provedor de dados é selecionado como aquele com a versão inicial mais alta.
Quando todos os 24 nós periféricos atingem o centro, a simulação regista o instante de início da sincronização e quando termina o tempo de término é registaado e a duração total do processo é calculada. Em termos funcionais, todos os nós atualizam sua versão de dados para a maior versão observada na rede, simulando o efeito da convergência proporcionada pelo SVS. O estado final de cada nó é então impresso e, para enriquecer a análise. Para simplificar esta analise podemos visualizar na figura \ref{fig:Fluxograma} um fluxograma do código feito.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{Chapters/Figures/Fluxograma.png}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Para demonstrar a diferença entre a sincronização normal num cruzamento, e a sincronização de com pivôs vão ser apresentados os resultados das duas implementações. Para simular os resultados foram utilizados 24 nós.
Para começar apresentam-se os resultados da simulação sem pivô.
Na figura \ref{fig:SVSP} temos o vetor de estado final da implementação. Como podemos ver tudo correu bem pois a versão final corresponde à versão mais recente em todos os nós da simulação.
\begin{figure}[h]
    \centering
    \includegraphics{Chapters/Figures/VersaoSP.png}
    \caption{Vetor de estado} 
    \label{fig:SVSP}   
\end{figure}

De seguida na fiugra \ref{fig:PacketsSP} podemos ver o número de pacotes que foram trocados, e isso vai ser muito importante para as conclusõees finais desta secção. Mas, por alto podemos ver um elevado número de pacotes, que se deve a colisões e perdas durante a sincronização, visto que o processo neste caso é bastante mais desorganizado.
\begin{figure}[h]
    \centering
    \includegraphics{Chapters/Figures/Packetssp.png}
    \caption{Número de pacotes trocados.} 
    \label{fig:PacketsSP}   
\end{figure}

Passando agora para os resultados da simulação que implementa a solução proposta.
Começando tambem com o vetor de estado final representado na figura \ref{fig:SVCP}, podemos também comprovar que realmente a troca de dados está a acontecer de forma correta, pois a versão final corresponde novamente à mais recente.
\begin{figure}[h]
    \centering
    \includegraphics{Chapters/Figures/VersaoCP.png}
    \caption{Vetor de estado final.} 
    \label{fig:SVCP}   
\end{figure}

Na figura \ref{fig:provedor} podemos ver quem foi o provedor dos dados mais recentes, e também a comprovar a existência de um pivô por cada faixa, que irá ser o ponto fulcral da diferença esperada entre as 2 soluções.
\begin{figure}[h]
    \centering
    \includegraphics{Chapters/Figures/Pivoeprovedor.png}
    \caption{Pivôs e provedor.}
    \label{fig:provedor}   
\end{figure}

Na figura \ref{fig:PacketsCp} podemos ver o valor, que continua algo alto por ainda existirem colisões e perdas.
\begin{figure}[h]
    \centering
    \includegraphics{Chapters/Figures/Packetscp.png}
    \caption{Número de pacotes trocados} 
    \label{fig:PacketsCp}   
\end{figure}
